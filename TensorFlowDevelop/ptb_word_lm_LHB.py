...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    configaaa = tf.ConfigProto()
    configaaa.gpu_options.per_process_gpu_memory_fraction = 0.95  # 占用GPU90%的显存
    configaaa.gpu_options.allow_growth = True  # 占用GPU9的显存可以增加
    configaaa.allow_soft_placement = True
    config.log_device_placement = False

    with sv.managed_session(config=configaaa) as session:
